#include "include.h"
#if ROBOT_Lite12_ONNX // YH lite onnx good 12 action zhenglaoshi
#include "gait_math.h"
#include "eso.h"
...
void Gait_RL_Update_2(float dt) // 2ms
{
    ...
    if (timer[0] > vmc_all.net_run_dt1-0.001 && rl_model_loaded)
    { //
        float cmd_x_temp = 0;
        float cmd_y_temp = 0;
        ...
        q_now[4] = q_convert[0][0] * 57.3;
        q_now[5] = q_convert[0][1] * 57.3; // degreed
        q_now[10] = q_convert[1][0] * 57.3;
        q_now[11] = q_convert[1][1] * 57.3;
        qd_now[4] = qd_convert[0][0] * 57.3;
        ...
        {
            obs_now[i] = LIMIT(obs_now[i], -100, 100);
        }
        ...
        for (int i = 0; i < num_single_obs ; i++)
        {
            action_onnx[i] = LIMIT(action_onnx[i], -3, 3);
        }

        action_onnx[2] = LIMIT(action_onnx[2], -1, 1); //hip yaw
        action_onnx[8] = LIMIT(action_onnx[8], -1, 1);
        action_onnx[5] = LIMIT(action_onnx[5], -0.1, 0.1); //ankle rol
        action_onnx[11] = LIMIT(action_onnx[11], -0.1, 0.1); //ankle rol
        actionOld_onnx = action_onnx;

        for (int i = 0; i < 12; i++)
        {
            if(fabs(action_onnx[i])>3&&0)
    ...
    for(int i=0;i<12;i++)
        nav_rx.temp_recordi[14+i]=leg_motor_all.q_now[i]/ 57.3;
    for(int i=0;i<12;i++)
        nav_rx.temp_recordi[14+12+i]=leg_motor_all.q_set[i]/ 57.3;
    for(int i=0;i<12;i++)
        nav_rx.temp_recordi[14+12+12+i]=leg_motor_all.qd_now[i]/ 57.3;
    for(int i=0;i<12;i++)
        nav_rx.temp_recordi[14+12+12+12+i]=leg_motor_all.t_now[i];

    for(int i=0;i<12;i++)
        nav_rx.temp_recordo[i]=action_onnx[i];
    nav_rx.temp_recordo[13]=t2 - t1;
#endif
...
#endif
    //ankle output
    set_convert[0][0] = LIMIT(actionFilt_out[4], -0.8, 1); // left
    set_convert[0][1] = LIMIT(actionFilt_out[5], -0.4, 0.4);
    set_convert[1][0] = LIMIT(actionFilt_out[10], -0.8, 1); // right
    set_convert[1][1] = LIMIT(actionFilt_out[11], -0.4, 0.4);
    ...
    ankle_lite_r.setTgtAnk(-set_convert[1][0], -set_convert[1][1],
                           temp, temp,
                           temp, temp);
    ankle_lite_l.getTgtMot(actionFilt_out[4], actionFilt_out[5],
                           temp, temp,
                           temp, temp);
    ankle_lite_r.getTgtMot(actionFilt_out[10], actionFilt_out[11],
                           temp, temp,
                           temp, temp);
