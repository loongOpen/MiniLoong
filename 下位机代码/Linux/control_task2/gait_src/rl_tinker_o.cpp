...
void RL_Modle_load_test(void)
{
    ...
    onnx.init("Model_YH/Trot_2/policy.onnx", "taitan_onnx");
...
void Gait_RL_Update_2(float dt) // 2ms
{
    ...
    for (int i = 0; i < 10; i++)
    {
        actionFilt_onnx[i] = LIMIT(actionFilt_onnx[i], -4, 4);
    }
    ...
    for(int i=0;i<14;i++){
        leg_motor_all.q_set_servo[i]=leg_motor_all.q_set_servo_init[i];//doghome
    }
#if 0
   //waist
   static float flt_yaw_rate=0;
   DigitalLPF(vmc_all.att_rate[YAWr] / 57.3,&flt_yaw_rate,0.5,dt);
   leg_motor_all.q_set[5]=flt_yaw_rate*8;
   //printf("leg_motor_all.q_set[6]=%f\n",leg_motor_all.q_set[5]);
