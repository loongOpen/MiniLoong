          ...
          if(ocu.mode==RC_REMOTE||ocu.esp32_connect){
             switch(vmc_all.gait_mode){
                 default:
                    ...
                    DigitalLPF(-ocu.rate_yaw_w*MAX_SPD_X/2,&vmc_all.tar_spd.y,FLT_RC,T);
                    DigitalLPF(-ocu.rc_att_w[ROLr]*MAX_SPD_RAD,&vmc_all.tar_spd.z,FLT_RC,T);
                 break;
             }
          }
        ...
        if(rc_interuped||sdk.cmd_vyaw==-1||sdk.sdk_mode==0){//rc interupte
            ...
            switch(vmc_all.gait_mode){
                default:
                   ...
                   DigitalLPF(ocu.rc_spd_w[Yr]*MAX_SPD/2,&vmc_all.tar_spd.y,FLT_RC/2,T);
...
#endif
#if !THREAD_1
    //pthread_create(&tid_t[0], NULL, Thread_RL, NULL);
