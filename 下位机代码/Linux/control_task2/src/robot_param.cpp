...
void robot_param_read(void)
{
    ...
    config_robot=YAML::LoadFile("/home/odroid/Tinker/Param/param_robot.yaml");
    config_gait=YAML::LoadFile("/home/odroid/Tinker/Param/param_gait.yaml");
...
//    }
    ...
    vmc_all.rl_flt=config_gait["rl_gait"]["rl_flt"].as<float>();
    ...
    printf("param rl-default loading... robotwb.type=%d\n",robotwb.type);

    if(robotwb.type==RL_BOY){
        printf("robotwb.type==RL_BOY\n");
        for(int i=0;i<14;i++){
            leg_motor_all.kp_servo[i]=2.0;
            leg_motor_all.kd_servo[i]=0.15;
            leg_motor_all.stiff_servo[i]=1.6;
        }
        leg_motor_all.arm_q_scale=0.58;
    }
    if(robotwb.type==LITE){
        printf("robotwb.type==Lite\n");
        for(int i=0;i<14;i++){
            leg_motor_all.kp_servo[i]=2.0;
            leg_motor_all.kd_servo[i]=0.15;
            leg_motor_all.stiff_servo[i]=1.6;
        }
        leg_motor_all.arm_q_scale=1;
    }
