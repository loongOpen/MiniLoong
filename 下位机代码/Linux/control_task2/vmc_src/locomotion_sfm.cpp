...
void reset_tar_pos(char sel){//now set is degree
    ...
    for(i=0;i<14;i++){//复位状态 for smooth
       ...
       leg_motor_all.q_set_servo[i]=leg_motor_all.q_servo[i];
    }
}
...
void locomotion_sfm(float dt)
{
  ...
  switch (gait_ww.state_gait)
  {
        case 0:
            ...
            if(ocu.key_x||ocu.key_b||get_new_sdk_input)timer[0]+=dt;else timer[0]=0;
            if(timer[0]>1.0)
            {
                #if RUN_WEBOTS
                ...
                #endif
                if(ocu.key_b==0)
                    leg_motor_all.motor_en=1;//开始能 leg
                leg_motor_all.servo_en=1;
                motor_power_on=1;
                timer[0]=timer[1]=0;
                vmc_all.param.soft_weight=1;
                ...
                if(robotwb.type==RL_BOY){
                    for(i=0;i<14;i++)//复位状态 for smooth
                        leg_motor_all.stiff_servo[i]=0.1;
                }
  ...
  case 2://步态线程------------------------<<
    ...
    switch(vmc_all.gait_mode){
        ...
        break;
        case G_RL:
            switch(vmc_all.rl_model_type){
                case 1:Gait_RL_Update_1(dt);break;
                case 2:Gait_RL_Update_2(dt);break;
            }
        break;
        case RECOVER:
    ...
    if(((ocu.connect&&ocu.mode>=2)||sdk.sdk_mode==1||ocu.esp32_connect)||(RUN_WEBOTS&&!RUN_PI)){//锟斤拷通锟街憋拷
            ...
            case 18://RL Locomotion-------------------------------
                ...
                if (((ocu.key_b == 1 && ocu.key_b_reg == 0) ||
                    ...
                    (sdk.sdk_mode == 1 && sdk.gait_mode == STAND_RC)) && vmc_all.rl_mode_used!=0)
                {
#if 0
                    stand_switch_trig = 0;
                    ocu.cmd_robot_state = 2;
                ...
                if (((ocu.key_x == 1 && ocu.key_x_reg == 0) ||
                    ...
                    (sdk.sdk_mode == 1 && sdk.gait_mode == G_RL)) && vmc_all.rl_mode_used!=0)
                {
                    timer_auto_switch=0;
               ...
               // printf("check_tau_knee=%f check_tau_ankle=%f\n",check_tau_knee,check_tau_ankle);
                if((ocu.key_a != 0 && ocu.key_a_reg == 0) &&0){
                    kick_lsm++;
                    kick_timer=0;
                }
