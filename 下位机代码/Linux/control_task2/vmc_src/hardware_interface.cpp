...
void set_motor_t_tinker(int id)
{
    ...
    temp_in[2]+=LIMIT(temp_att[2],-15,15);
        ...
        if(gait_ww.state_gait>=2){
            DigitalLPF(-ocu.rc_att_w[PITr]*60,&leg_motor_all.q_set[5],0.2,0.005);//pitch
            leg_motor_all.q_set[5]=LIMIT(leg_motor_all.q_set[5],-30,50);
            DigitalLPF(ocu.rc_att_w[ROLr]*100,&leg_motor_all.q_set[6],0.2,0.005);//yaw
            leg_motor_all.q_set[6]=LIMIT(leg_motor_all.q_set[6],-90,90);
            //printf("head=%f %f\n",leg_motor_all.q_set[5],leg_motor_all.q_set[6]);
        }
#endif
...
{
    static int cnt_p=0;
    static int cap_flag_test=0;
    static int key_reg=0;
    float temp_in[14]={0};
    if(ocu.key_ud==1&&key_reg==0)
    {
       if(cap_flag_test==1)cap_flag_test=0;
       else cap_flag_test=1;
    }
#if 1
//    q_set_servo[0]=30- sin(3.1415926 * 2 * vmc_all.rl_gait_time / vmc_all.rl_gait_duty)*10*flt_rate;//
//    q_set_servo[1]=-80;
//    q_set_servo[2]=0;
//    q_set_servo[3]=-50;
//    //right
//    q_set_servo[7]=30+ sin(3.1415926 * 2 * vmc_all.rl_gait_time / vmc_all.rl_gait_duty)*10*flt_rate;//
//    q_set_servo[8]=80;
//    q_set_servo[9]=0;
//    q_set_servo[10]=50;
    if(1||ocu.vr_connect==0||ocu.vr_enable==0){
        if(cap_flag_test==1){
            printf("cap open!\n");
            //leg_motor_all.reset_q_div_servo[3]=1;
            //leg_motor_all.reset_q_div[9]=1;            leg_motor_all.reset_q_div[10]=1;            leg_motor_all.reset_q_div[11]=1;
            //leg_motor_all.reset_q_div[2]=1;            leg_motor_all.reset_q_div[3]=1;            leg_motor_all.reset_q_div[4]=1;
            leg_motor_all.q_set_servo[12]=-25;
            leg_motor_all.q_set_servo[13]=25;
        }else{
            for(int i=0;i<14;i++)
                leg_motor_all.reset_q_div_servo[i]=0;
            for(int i=0;i<14;i++)
                leg_motor_all.reset_q_div[i]=0;
            //printf("cap close!\n");
            leg_motor_all.q_set_servo[12]=25;
            leg_motor_all.q_set_servo[13]=-25;
        }
    ...
    for(int i=0;i<14;i++){
       temp_in[i]=leg_motor_all.q_set_servo[i]+leg_motor_all.q_set_servo_off[i];
       DigitalLPF(temp_in[i]/leg_motor_all.arm_q_scale,&leg_motor_all.q_set_servo_out[i],3,0.001);
    }
    ...
    {
        cnt_p=0;
        printf("arm l:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_servo[0],leg_motor_all.q_servo[1],leg_motor_all.q_servo[2],leg_motor_all.q_servo[3]);
        printf("arm r:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_servo[7],leg_motor_all.q_servo[8],leg_motor_all.q_servo[9],leg_motor_all.q_servo[10]);
        printf("leg ls:%.1f %.1f %.1f %.1f %.1f \n",leg_motor_all.q_now[0],leg_motor_all.q_now[1],leg_motor_all.q_now[2],leg_motor_all.q_now[3],leg_motor_all.q_now[4]);
        printf("leg rs:%.1f %.1f %.1f %.1f %.1f \n",leg_motor_all.q_now[7],leg_motor_all.q_now[8],leg_motor_all.q_now[9],leg_motor_all.q_now[10],leg_motor_all.q_now[11]);
    }
#endif
}

void set_motor_t_lite(int id)
{
    static int cnt_p=0;
    ...
    if(cnt_p++>100)
    {
        ...
        printf("arm l:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_servo[0],leg_motor_all.q_servo[1],leg_motor_all.q_servo[2],leg_motor_all.q_servo[3]);
        printf("arm r:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_servo[7],leg_motor_all.q_servo[8],leg_motor_all.q_servo[9],leg_motor_all.q_servo[10]);
        printf("arm ls:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_set_servo[0],leg_motor_all.q_set_servo[1],leg_motor_all.q_set_servo[2],leg_motor_all.q_set_servo[3]);
        printf("arm rs:%.1f %.1f %.1f %.1f \n",leg_motor_all.q_set_servo[7],leg_motor_all.q_set_servo[8],leg_motor_all.q_set_servo[9],leg_motor_all.q_set_servo[10]);
    }
#endif
}
