...
// 
	...
	&& ((RxMessage.DLC == 6||RxMessage.DLC == 8))) /* 数据长度为8 */
	{
		...
		&& ((RxMessage.DLC >=6)) &&
			motor_chassis[0].motor.type>=EC_1) /* 数据长度为8 */
		{
			RV_can_data_repack(&RxMessage,0,1);
			for(i=7;i<14;i++){
				leg_motor.q_now[i+7]=motor_chassis[i+7].q_now_flt;
				leg_motor.qd_now[i+7]=motor_chassis[i+7].qd_now_flt;
				leg_motor.t_now[i+7]=motor_chassis[i+7].t_now_flt;
			}
		}			
...
void CAN_motor_sm(float dt)
{
	char i=0;
	char id_cnt=0;
	if(ocu_connect==0){//no ocu
		for(i=0;i<14;i++){
			leg_motor.motor_en=palm_dj.power*palm_dj.connect; 
			
			motor_chassis[i].set_q=palm_dj.q_exp[i]; 
			
			motor_chassis[i].cal_div=palm_dj.cal_div[i];
			motor_chassis[i].param.q_reset_angle=palm_dj.q_reset[i];
			
			motor_chassis[i].stiff=1; 
			motor_chassis[i].kp=palm_dj.kp[i];
			motor_chassis[i].kd=palm_dj.kd[i];

			motor_chassis[i].param.usb_cmd_mode=1;//设置为位置模式		
		}
	}
	
	if(motor_chassis[0].motor.type==TAIHU_1)//--------- TAIHU电机 motor.type 是读取的上位机配置
	{
		if(!can_cmd_usb_disable||ocu_connect==0)//usb没连接
		{
			for(i=0;i<14;i++)
				motor_chassis[i].en_cmd=leg_motor.motor_en;
				mit_bldc_thread_taihu(leg_motor.motor_en,dt);
		}
		else
		{
			for(i=0;i<14;i++)
			{
				...
				mit_bldc_thread_taihu(motor_chassis[0].en_cmd_ocu,dt);
			}
		}
	}
	else if(motor_chassis[0].motor.type>=EC_1)//----------ECOS电机 motor.type 是读取的上位机配置
	{
		if(!can_cmd_usb_disable||ocu_connect==0)//usb没连接
		{
			for(i=0;i<14;i++)
