...
class LeggedRobotXd(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.feet_height1 = torch.zeros((self.num_envs, 2), device=self.device)
        self.feet_height2 = torch.zeros((self.num_envs, 2), device=self.device)
        self.last_feet_z = torch.zeros((self.num_envs, 2), device=self.device)#0.05
        self.last_feet_z1 = torch.zeros((self.num_envs, 2), device=self.device)#0.05
        self.last_feet_z2 = torch.zeros((self.num_envs, 2), device=self.device)#0.05
    ...
    def compute_observations(self):#guance
        ...
        noise_vec = torch.cat((torch.ones(3) * noise_scales.ang_vel * noise_level,
                               ...
                               torch.zeros(2),#need fix for sin cos
                               ), dim=0)
        ...
        if 0:#Xd -----------------------------------------------------
            ...
            priv_latent = torch.cat((#
                ...
                self.reindex_feet(self.contact_filt.float()),
                self.randomized_lag_tensor,
    ...
    def _get_gait_phase(self):
        ...
        # Double support phase 双支撑
        stance_mask[torch.abs(sin_pos) <  self.cfg.rewards.stand_double] = 1
    ...
    def _get_base_heights(self, env_ids=None):
        ...
        return base_height
    #------------ reward functions----------------jiangliXd

    def _reward_feet_contact_number_xd(self):
        """
    ...
    def _reward_joint_ref_pos_xd(self):# 期望关节角度轨迹
        """
    ...
    def _reward_feet_air_time_xd(self):
        """
        ...
        contact = self.contact_forces[:, self.feet_indices, 2] > self.cfg.rewards.touch_thr
        stance_mask = self._get_gait_phase()
        self.contact_filt1 = torch.logical_or(torch.logical_or(contact, stance_mask), self.last_contacts1)
        self.last_contacts1 = contact
        first_contact = (self.feet_air_time1 > 0.) * self.contact_filt1
        self.feet_air_time1 += self.dt
        air_time = self.feet_air_time1.clamp(0, 0.5) * first_contact
        self.feet_air_time1 *= ~self.contact_filt1
        return air_time.sum(dim=1)

    def _reward_feet_clearance_xd(self):
        """
        ...
        delta_z = feet_z - self.last_feet_z1
        self.feet_height1 += delta_z
        self.last_feet_z1 = feet_z

        # Compute swing mask
        swing_mask = 1 - self._get_gait_phase()

        # feet height should be closed to target feet height at the peak
        rew_pos = torch.abs(self.feet_height1 - self.cfg.rewards.target_feet_height) < 0.01
        rew_pos = torch.sum(rew_pos * swing_mask, dim=1)
        self.feet_height1 *= ~contact
        return rew_pos

    def _reward_feet_swing_trajectory_z_xd(self):#gaodu
        ...
        print("trajectory_z:=",self.leg_phase[0],self.target_height[0],self.feet_pos[:, 0])
        return res

    def _reward_feet_swing_height_xd(self):
        res = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        ground_height = self._get_heights()
        for i in range(2):
    ...
    def _reward_no_jump(self):
        ...
        return reward_out2#+reward_out3
    ...
    def _cost_feet_air_time_xd(self):
        """
        Calculates the reward for feet air time, promoting longer steps. This is achieved by
        checking the first contact with the ground after being in the air. The air time is
        limited to a maximum value for reward calculation.
    ...
    def _cost_feet_contact_number_xd(self):
        """
        Calculates a reward based on the number of feet contacts aligning with the gait phase. 
        Rewards or penalizes depending on whether the foot contact matches the expected gait phase.
        """
